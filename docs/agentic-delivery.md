...
- Each channel has one reader:
  - **Telegram**: `listen.py`, always on (k3s Deployment `gatekeeper-listener`, SB-951), is the **only** `getUpdates` reader (`allowed_updates: [callback_query, message]`). `from.id` ∈ allowlist, `callback_data = gate_id:verb`. A tap is recorded on the gate (`pending_decision`), `answerCallbackQuery`'d within seconds, then applied; if Linear is down it stays pending and is retried. An update is acked only once recorded. "Note" → next text from allowed user attaches. Free-text `approve` / `reject: reason` also accepted; other free text goes to the Maildir inbox `$STATE/inbox/telegram/` for the PO chat (SB-1089).
  - **Linear**: `gate.py poll`, on the runner's tick — comments on the issue after the marker, by the assignee. First line `approve`, `approve: note`, `reject: reason`. Any other comment = discussion → forwarded to Telegram, gate stays open (agent may reply in-thread via `--resume` if the comment is a question — cap at 2 exchanges before `needs-human`).
...
- Handoff: every decision sets `handoff: pending` on the gate; the runner's `gate.py poll --once` returns each such gate once and marks it `claimed`. `poll` never calls `getUpdates` — a second reader on one token is a 409.
